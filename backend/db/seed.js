...
function seed() {
  ...
  const upsertUser = (username, email, password, role) => {
    const existing = db.prepare('SELECT id FROM users WHERE username = ?').get(username);
    if (existing) {
      // Make sure role is set on previously seeded users
      db.prepare('UPDATE users SET role = ? WHERE id = ? AND (role IS NULL OR role = ?)').run(role, existing.id, '');
      return existing.id;
    }
    const hashed = bcrypt.hashSync(password, 10);
    return db.prepare(
      'INSERT INTO users (username, email, password, role) VALUES (?, ?, ?, ?)'
    ).run(username, email, hashed, role).lastInsertRowid;
  };

  // owner: full read/write access; viewer: read-only access
  const ownerId = upsertUser('demo', 'demo@example.com', 'demo123', 'owner');
  const viewerId = upsertUser('demoviewer', 'demoviewer@example.com', 'demo123', 'viewer');
  ...
  const createLinks = (userId, categoryIds) => {
    ...
    links.forEach((link) => insertLinkWithTags(link));
  };

  const seedDataForUser = (userId) => {
    const existing = db.prepare('SELECT COUNT(*) as count FROM links WHERE user_id = ?').get(userId);
    if (existing.count > 0) {
      console.log(`User ${userId} already has links, skipping.`);
      return;
    }
    createLinks(userId, createCategories(userId));
    console.log(`Seeded ${categories.length} categories and ${links.length} links for user ${userId}.`);
  };

  seedDataForUser(ownerId);
  // Give the read-only viewer something to view, but no ability to modify
  seedDataForUser(viewerId);

  console.log('Seed data created successfully!');
  console.log('- demo/demo123 (owner, read/write)');
  console.log('- demoviewer/demo123 (viewer, read-only)');
}
