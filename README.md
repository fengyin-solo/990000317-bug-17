...
- 管理员（可读写）: 用户名 `demo` / 密码 `demo123`
- 只读账号（仅可查看）: 用户名 `demoviewer` / 密码 `demo123`

### 登录态与权限

- 登录信息保存在本地，刷新页面会在首次路由前同步恢复，并调用 `/api/auth/me` 向服务端复核当前角色。
- 令牌过期、失效或被服务端拒绝时，顶部导航会立即切回未登录状态并跳转到登录页说明原因（无需手动触发操作）。
- 多标签页中任一标签退出登录，其他标签会同步退出。
- 导航入口按当前使用者权限渲染：只读账号看不到“导入书签”，“死链检测/稍后阅读”标注“只读”，页面内的写操作按钮同步禁用；后端对所有写接口同样强制校验，越权返回 403。
...
### 认证
- `POST /api/auth/register` - 用户注册（注册即为自己数据的 owner）
- `POST /api/auth/login` - 用户登录
- `GET /api/auth/me` - 获取当前登录用户（刷新后复核角色）
